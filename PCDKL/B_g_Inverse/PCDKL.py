...
def pde_fn(x, u, k):
    ...
    u_t = torch.unsqueeze(u_x[:, 0], dim=-1)
    u_X = torch.unsqueeze(u_x[:, 1], dim=-1)
    u_XX = torch.unsqueeze(u_xx[:, 1], dim=-1)
...
class PCDKL:
    def __init__(self, cfg):
        ...
        self.log_k = torch.tensor([0.], dtype=torch.float64, requires_grad=True)
        ...
        # the optimizer
        self.params = [self.log_ls, self.log_k] + list(self.feature_extractor.parameters())
    ...
    def train(self):
        ...
        for i in range(self.epoch):
            ...
            print(torch.exp(self.log_k) * 1000)
    ...
    def get_ELBO(self):
        ...
        for i in range(self.num_samples):
            ...
            u = u_mean + u_std * eta
            f = self.pde_fn(self.x_f_train, u, torch.exp(self.log_k))
...
if __name__ == '__main__':
    ...
    dataset_filename = os.path.join(base_dir, "PCDKL/B_g_Inverse/dataset0.01.mat")
    ...
    # plot the results
    u_title = 'PCDKL'
    error_u = np.abs(u_mean - u_test).reshape(100, 100)
    std_u_2 = (u_std * 2).reshape(100, 100)
    u_mean = u_mean.reshape(100, 100)
